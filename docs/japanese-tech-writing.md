# 日本語技術文書の文章規範（ライティング基準）

> 出典: k16shikano / `japanese-tech-writing` (SKILL.md)
> https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d
> 講義本文の執筆・リライト（Issue #35）の文章規範として使用する。LLM に「LLMっぽい日本語」を生成させないための規範であり、人間が書くときの禁則集ではない。
...
LLM が大量生成する、中身のない型に誘惑されない。書き上げたら、この節で点検する。術語を議論に使うのはよい。空虚な装飾として使うのが問題である。
